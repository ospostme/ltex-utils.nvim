...
local function ltex_filename(absolutePath)
  local dir
  if Config.ltex_json then
    dir = Config.ltex_json
  else
    dir = vim.fn.fnamemodify(absolutePath, ':h')
  end

  local name = vim.fn.fnamemodify(absolutePath, ':t:r')
  local ext = vim.fn.fnamemodify(absolutePath, ':e')
  local newName = name .. '_' .. ext .. '_ltex.json'

  if dir and dir ~= '.' and dir:sub(-1) ~= '/' then
    dir = dir .. '/'
  end

  return (dir and dir ~= '.' and dir or '') .. newName
end
...
function M.write_ltex_to_file(bufnr)
  ...
  if settings_to_save then
    ...
    if Config.ltex_json then
      settings_io.ensure_folder_exists(Config.ltex_json)
    end
