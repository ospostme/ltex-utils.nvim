...
local defaults = {
  ...
  backend = 'ltex_plus',
  -- path to the directory where ltex json files are stored
  ltex_json = nil,
}
...
local function get_vim_dict_dir(custom_path)
  ...
  if rtp and rtp ~= '' then
    ...
    for _, path in ipairs(paths) do
      ...
      if expanded_path and expanded_path ~= '' then
        ...
        -- create it
        if vim.fn.filewritable(expanded_path) == 2 and vim.fn.isdirectory(spell_dir) == 0 then
  ...
  vim.notify(
    'Could not find valid directory for writing vim dictionary. ' .. 'All dictionary data held by ltex will be lost. We are sorry.',
...
function M.setup(opts)
  opts = opts or {}
  local vim_dict_settings = opts.dictionary
      and opts.dictionary.use_vim_dict
      and {
        dictionary = {
          path = get_vim_dict_dir(vim.fn.stdpath 'config' .. '/spell/'),
          filename = function(lang)
            local fileencoding = vim.api.nvim_get_option_value('fileencoding', { buf = 0 }) or 'utf-8'
